...
set.seed(90015)
system.time( bootCI.sc3 <- foreach(1:500) %dorng% {simulation.bootCI.fct(B=300, N=1500, fup=240, incidence.rate=0.333, 
                                                        expoGen="Duration", 
                                                        beta.curUse=log(3), beta.cumDur12w=log(2)/6, 
                                                        beta.sex=0.8, beta.age=0.05, 
                                                        addTVcov=0, beta.TVcov=NULL,
                                                        P=10, move2ndLast.SIMEX=seq(2, 30, by=2)) } )
save(bootCI.sc3, file="Sc3 - N1500 expoDuration BcurUse11 BcumDur12w012 - BootCI 300resamples - 500samples.RData")



###############################################################################
### Results for Tables and Figures in the manuscript and Supporting Information 
###############################################################################
...
mtext("a)", side=3, line=3, at=-9, cex=1.3)
...
mtext("b)", side=3, line=3, at=-9, cex=1.3)

dev.off()

# Mean difference between the imputed vs. the true event times, for both  
# exposure metrics, and corresponding values on the x axis of Figure above
mean(mean.dist2lastV.curUse[,1])
mean(mean.dist2lastV.curUse[,1])/4
mean(mean.dist2lastV.cumDur12w[,1])
mean(mean.dist2lastV.cumDur12w[,1])/4

# Mean difference between the last 2 visits at k=15 or k=11 iterations 
# (depending on the scenario) for current use, and at k=25 iterations, 
# and corresponding values on the x axis of Figure above

mean(mean.dist2lastV.curUse[,1+15])
mean(mean.dist2lastV.curUse[,1+15])/4
mean(mean.dist2lastV.cumDur12w[,1+low.nbr.it.cumDur12w])
mean(mean.dist2lastV.cumDur12w[,1+low.nbr.it.cumDur12w])/4

mean(mean.dist2lastV.curUse[,1+25])
mean(mean.dist2lastV.curUse[,1+25])/4
mean(mean.dist2lastV.cumDur12w[,1+25])
mean(mean.dist2lastV.cumDur12w[,1+25])/4
...
#-----------------------------------------------------------------------------#
# Coverage rates of 95% bootstrap confidence intervals                        # 
#-----------------------------------------------------------------------------#

rm(list=ls())

load("Sc3 - N1500 expoDuration BcurUse11 BcumDur12w012 - BootCI 300resamples - 500samples.RData")
 
### Functions ###

extract.fct <- function(output, argument){
  output[[argument]]
}

coverage.bootCI.fct <- function(output, argument, beta){
  # Extract upper and lower bounds of CIs
  bootCI.lb <- unlist(lapply(output, extract.fct, argument))[seq(1, length(output)*2, by=2)]
  bootCI.ub <- unlist(lapply(output, extract.fct, argument))[seq(2, length(output)*2, by=2)]
  # Coverage rate
  coverage <- sum(bootCI.lb <= beta & beta <= bootCI.ub)/length(output) * 100
  return(coverage)
}

coverage.normCI.fct <- function(output, argument, beta){
  # Extract parameter estimate and standard error
  est <- unlist(lapply(output, extract.fct, argument))[seq(1, length(bootCI.sc3)*6, by=6)]
  se <- unlist(lapply(output, extract.fct, argument))[seq(4, length(bootCI.sc3)*6, by=6)]
  # Calculate normal approximation of CI
  normCI.lb <- est - qnorm(0.975)*se 
  normCI.ub <- est + qnorm(0.975)*se 
  # Coverage rate
  coverage <- sum(normCI.lb <= beta & beta <= normCI.ub)/length(output) * 100
  return(coverage)
}

### SIMEX models, bootstrap CI ###

coverage.bootCI.fct(output = bootCI.sc3, argument = 'cox.curUse.SIMEX.bootCI',
                  beta = log(3))
coverage.bootCI.fct(output = bootCI.sc3, argument = 'cox.cumDur12w.SIMEX.bootCI',
                  beta = log(2)/6)

### Conventional Cox models, bootstrap CI ###

coverage.bootCI.fct(output = bootCI.sc3, argument = 'cox.curUse.mid.bootCI',
                  beta = log(3))
coverage.bootCI.fct(output = bootCI.sc3, argument = 'cox.cumDur12w.mid.bootCI',
                  beta = log(2)/6)

### Conventional Cox models, normal approximation CI ###

coverage.normCI.fct(output = bootCI.sc3, argument = 'cox.curUse.coef.mid',
                    beta = log(3))
coverage.normCI.fct(output = bootCI.sc3, argument = 'cox.cumDur12w.coef.mid',
                    beta = log(2)/6)

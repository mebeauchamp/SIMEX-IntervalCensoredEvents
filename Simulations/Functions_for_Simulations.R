...
#   visit before and firstvisit after the true event time                     #
#-----------------------------------------------------------------------------#
...
## Arguments:
  ...
  # addTVcov: simulation setting for additonal time-varying covariate
...
#   Data preparation for SIMEX method in the simualtions: adjust follow-up    #    
...
## Arguments:
  ...
  # seed: seed for the random number generator (to allow reproducability of 
  #       results).
...
simulation.fct <- function(n.sims, N, fup, incidence.rate, expoGen,  
  ...
  ## Correlation of exposures at different times for chosing extrapolation function  
  ...
  for (s in 1:n.sims){
    ...
    # Event times followed exponentiel distribution 
    ...
    # Prepartion of full history time-varying variables 
    if (addTVcov == 0){
    ...
    #############################################################################
    # Correlation of exposures at different times for chosing the extrapolation #
    # function                                                                  #
    #############################################################################

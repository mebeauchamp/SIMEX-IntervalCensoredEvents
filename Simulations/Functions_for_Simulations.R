...
#   for all samples for a given simualtion scenario.                          #
#   Note: 95% bootstrap CIs are not calculated by this function, see the      #
#   function simulation.bootCI.fct.                                           #
#-----------------------------------------------------------------------------#
...
#-----------------------------------------------------------------------------#
# simulation.bootCI.fct:                                                      #
#   Simulation function to generate data, estimate models, and calculate      #
#   bootstrap confidence intervals for:                                       #
#    i) conventional Cox model with event times imputed at middle of interval #
#       between the last 2 physician visits, and                              #
#    ii) SIMEX-like proposed method                                           #
#  for 1 sample for  a given simualtion scenario.                             #
#-----------------------------------------------------------------------------#
...
simulation.bootCI.fct <- function(B, N, fup, incidence.rate, expoGen,  
                                  beta.curUse, beta.cumDur12w, beta.sex, beta.age, 
                                  addTVcov, beta.TVcov = NULL, 
                                  P, move2ndLast.SIMEX){
  ...
  ### Create datasets with event times imputed at the middle of interval ###
  ### between last 2 visits                                              ###
  ...
  mean.dist2lastV.curUse <- rep(NA, times=length(move2ndLast.SIMEX))
  mean.dist2lastV.curUse[1] <- 
    mean(dat.curUse.mid[dat.curUse.mid$event==1,'vis.Last'] - 
           dat.curUse.mid[dat.curUse.mid$event==1,'vis.2ndLast'])
  mean.dist2lastV.cumDur12w <- rep(NA, times=length(move2ndLast.SIMEX))
  mean.dist2lastV.cumDur12w[1] <- 
    mean(dat.cumDur12w.mid[dat.cumDur12w.mid$event==1,'vis.Last'] - 
           dat.cumDur12w.mid[dat.cumDur12w.mid$event==1,'vis.2ndLast'])
  ...
  cox.curUse.coef.mid <- summary(coxph(
    as.formula(paste0('Surv(start, stop, event) ~ curUse +', covariates)), 
    data=dat.curUse.mid))$coef[,c(1,3)]
  cox.cumDur12w.coef.mid <- summary(coxph(
    as.formula(paste0('Surv(start, stop, event) ~ cumDur12w +', covariates)), 
    data=dat.cumDur12w.mid))$coef[,c(1,3)] 
  ...
  cox.curUse.move2ndLast.SIMEX <- rep(NA, length(move2ndLast.SIMEX))
  cox.cumDur12w.move2ndLast.SIMEX <- rep(NA, length(move2ndLast.SIMEX))  
  ...
  for (j in 1:length(move2ndLast.SIMEX)){
    ...
    # Step 2: Refit Cox PH model on the modified dataset
    cox.curUse.move2ndLast.SIMEX[j] <- coxph(
      as.formula(paste0('Surv(start, stop, event) ~ curUse +', covariates)), 
      data=dat.curUse.mid.modif)$coef[1]    
    cox.cumDur12w.move2ndLast.SIMEX[j] <- coxph(
      as.formula(paste0('Surv(start, stop, event) ~ cumDur12w +', covariates)), 
      data=dat.cumDur12w.mid.modif)$coef[1]   
    
    # Modified mean time between visits for cases 
    mean.dist2lastV.curUse[1+j] <- mean(
      dat.curUse.mid.modif[dat.curUse.mid.modif$event==1,'vis.Last'] - 
        dat.curUse.mid.modif[dat.curUse.mid.modif$event==1,'vis.2ndLastSIMEX'])
    mean.dist2lastV.cumDur12w[1+j] <- mean(
      dat.cumDur12w.mid.modif[dat.cumDur12w.mid.modif$event==1,'vis.Last'] - 
        dat.cumDur12w.mid.modif[dat.cumDur12w.mid.modif$event==1,'vis.2ndLastSIMEX']) 
  }
  ...
  # Mean difference between the (modified) times of the two relevant visits for cases
  x.lin.curUse <- mean.dist2lastV.curUse
  x.lin.cumDur12w <- mean.dist2lastV.cumDur12w
  
  # Coefficients from Cox PH models for modified events and censoring times 
  cox.curUse.expo.SIMEX <- c(cox.curUse.coef.mid[1,1], 
                             cox.curUse.move2ndLast.SIMEX)
  cox.cumDur12w.expo.SIMEX <- c(cox.cumDur12w.coef.mid[1,1], 
                                cox.cumDur12w.move2ndLast.SIMEX)
  
  # Step 3: Linear regression
  curUse.expo.SIMEXlin <- lm(cox.curUse.expo.SIMEX ~ x.lin.curUse)
  cumDur12w.expo.SIMEXlin <- lm(cox.cumDur12w.expo.SIMEX ~ x.lin.cumDur12w)

  # Step 4: Extrapolation
  cox.curUse.expo.SIMEXlin.extrap <- curUse.expo.SIMEXlin$coef[1] + 
    1*curUse.expo.SIMEXlin$coef[2]  
  cox.cumDur12w.expo.SIMEXlin.extrap <- cumDur12w.expo.SIMEXlin$coef[1] + 
    1*cumDur12w.expo.SIMEXlin$coef[2] 
  
  ####################################################
  # Bootstrap CI for conventional Cox & SIMEX models #
  ####################################################
  
  bcox.curUse.coef.mid <- rep(NA, times=B)
  bcox.cumDur12w.coef.mid <- rep(NA, times=B) 
    
  bcox.curUse.expo.SIMEXlin.extrap <- rep(NA, times=B) 
  bcox.cumDur12w.expo.SIMEXlin.extrap <- rep(NA, times=B)
  ...
  for (b in 1:B){
    ...
    ID.resamp <- sort(sample(1:N, replace=TRUE))
    # Duplicated id are ignored
    bdat.curUse.mid <- dat.curUse.mid[dat.curUse.mid$id %in% ID.resamp,] 
    bdat.cumDur12w.mid <- dat.cumDur12w.mid[dat.cumDur12w.mid$id %in% ID.resamp,]
    # Number of times this id has appeared in the resample up to now
    bdat.curUse.mid$bdup <- 1    
    bdat.cumDur12w.mid$bdup <- 1 
    
    # Deal with duplicated id
    step <- 2
    repeat {
      # Select duplicated id only
      ID.resamp <- ID.resamp[duplicated(ID.resamp)==TRUE]
      if (length(ID.resamp)==0) break # Break loop when no more duplicated id
      rep.curUse.mid <- dat.curUse.mid[dat.curUse.mid$id %in% ID.resamp, ]
      rep.curUse.mid$bdup <- step
      rep.curUse.mid$id <- rep.curUse.mid$id + step*10000        
      rep.cumDur12w.mid <- dat.cumDur12w.mid[dat.cumDur12w.mid$id %in% ID.resamp,]
      rep.cumDur12w.mid$bdup <- step
      rep.cumDur12w.mid$id <- rep.cumDur12w.mid$id + step*10000   
      bdat.curUse.mid <- rbind(bdat.curUse.mid, rep.curUse.mid)
      bdat.cumDur12w.mid <- rbind(bdat.cumDur12w.mid, rep.cumDur12w.mid)
      step <- step + 1
    }
    
    ### Distance between last 2 visits for case (before and after true event time) ###
    
    bmean.dist2lastV.curUse <- rep(NA, times=(1+length(move2ndLast.SIMEX))) 
    bmean.dist2lastV.cumDur12w <- rep(NA, times=(1+length(move2ndLast.SIMEX)))
    
    bmean.dist2lastV.curUse[1] <- 
      mean(bdat.curUse.mid[bdat.curUse.mid$event==1,'vis.Last'] - 
             bdat.curUse.mid[bdat.curUse.mid$event==1,'vis.2ndLast'])
    bmean.dist2lastV.cumDur12w[1] <- 
      mean(bdat.cumDur12w.mid[bdat.cumDur12w.mid$event==1,'vis.Last'] - 
             bdat.cumDur12w.mid[bdat.cumDur12w.mid$event==1,'vis.2ndLast'])
    
    ### Conventional Cox models with event times imputed at middle interval ###
    
    bcox.curUse.coef.mid[b] <- summary(coxph(
      as.formula(paste0('Surv(start, stop, event) ~ curUse +', covariates)), 
      data=bdat.curUse.mid))$coef[1,1]
    bcox.cumDur12w.coef.mid[b] <- summary(coxph(
      as.formula(paste0('Surv(start, stop, event) ~ cumDur12w +', covariates)), 
      data=bdat.cumDur12w.mid))$coef[1,1] 
    
    ### SIMEX-like proposed method ###
    
    bcox.curUse.move2ndLast.SIMEX <- rep(NA, times=length(move2ndLast.SIMEX))  
    bcox.cumDur12w.move2ndLast.SIMEX <- rep(NA, times=length(move2ndLast.SIMEX)) 
    
    for (j in 1:length(move2ndLast.SIMEX)){
      
      ## Step 1: Gradually increase mean time between visits for cases by moving 
        #        backward 2nd last visits. Move backward last visits for controls.
      bdat.curUse.mid.modif <- SIMEX_dataPrepSims.fct(dat = bdat.curUse.mid, 
                                                     back.units = move2ndLast.SIMEX[j])
      bdat.cumDur12w.mid.modif <- SIMEX_dataPrepSims.fct(dat = bdat.cumDur12w.mid, 
                                                        back.units = move2ndLast.SIMEX[j])
  
      ## Step 2: Refit Cox Ph model on the modified dataset
      bcox.curUse.move2ndLast.SIMEX[j] <- coxph(
        as.formula(paste0('Surv(start, stop, event) ~ curUse +', covariates)), 
        data = bdat.curUse.mid.modif)$coef[1]    
      bcox.cumDur12w.move2ndLast.SIMEX[j] <- coxph(
          as.formula(paste0('Surv(start, stop, event) ~ cumDur12w +', covariates)), 
        data = bdat.cumDur12w.mid.modif)$coef[1]   
      
      # Modified mean time between visits for cases 
      bmean.dist2lastV.curUse[1+j] <- mean(
        bdat.curUse.mid.modif[bdat.curUse.mid.modif$event==1,'vis.Last'] - 
          bdat.curUse.mid.modif[bdat.curUse.mid.modif$event==1,'vis.2ndLastSIMEX'])
      bmean.dist2lastV.cumDur12w[1+j] <- mean(
        bdat.cumDur12w.mid.modif[bdat.cumDur12w.mid.modif$event==1,'vis.Last'] - 
          bdat.cumDur12w.mid.modif[bdat.cumDur12w.mid.modif$event==1,'vis.2ndLastSIMEX']) 
    }
    
    ## Step 3: Linear regression
    bcox.curUse.expo.SIMEX <- c(bcox.curUse.coef.mid[b], 
                                bcox.curUse.move2ndLast.SIMEX)
    bcox.cumDur12w.expo.SIMEX <- c(bcox.cumDur12w.coef.mid[b], 
                                   bcox.cumDur12w.move2ndLast.SIMEX)
    
    bcurUse.expo.SIMEXlin <- lm(bcox.curUse.expo.SIMEX ~ bmean.dist2lastV.curUse)
    bcumDur12w.expo.SIMEXlin <- lm(bcox.cumDur12w.expo.SIMEX ~  bmean.dist2lastV.cumDur12w)
    
    ## Step 4: Extrapolation
    bcox.curUse.expo.SIMEXlin.extrap[b] <- bcurUse.expo.SIMEXlin$coef[1] + 
      1*bcurUse.expo.SIMEXlin$coef[2]  
    bcox.cumDur12w.expo.SIMEXlin.extrap[b] <- bcumDur12w.expo.SIMEXlin$coef[1] + 
      1*bcumDur12w.expo.SIMEXlin$coef[2] 
  }
  
  ### Bootstrap 95% CI ###
  
  # Concentional Cox models
  cox.curUse.mid.bootCI <- quantile(bcox.curUse.coef.mid, probs=c(0.025, 0.975))
  cox.cumDur12w.mid.bootCI <- quantile(bcox.cumDur12w.coef.mid, probs=c(0.025, 0.975))   
  
  # SIMEX-like proposed method
  cox.curUse.SIMEX.bootCI <- quantile(bcox.curUse.expo.SIMEXlin.extrap, probs=c(0.025, 0.975))
  cox.cumDur12w.SIMEX.bootCI <- quantile(bcox.cumDur12w.expo.SIMEXlin.extrap, probs=c(0.025, 0.975))         

  ##################
  # Return results #
  ##################
  
  return(list(cox.curUse.coef.mid = cox.curUse.coef.mid,
              cox.cumDur12w.coef.mid = cox.cumDur12w.coef.mid,
              cox.curUse.expo.SIMEXlin.extrap = cox.curUse.expo.SIMEXlin.extrap, 
              cox.cumDur12w.expo.SIMEXlin.extrap = cox.cumDur12w.expo.SIMEXlin.extrap,
              cox.curUse.mid.bootCI = cox.curUse.mid.bootCI,
              cox.cumDur12w.mid.bootCI = cox.cumDur12w.mid.bootCI,      
              cox.curUse.SIMEX.bootCI = cox.curUse.SIMEX.bootCI,
              cox.cumDur12w.SIMEX.bootCI = cox.cumDur12w.SIMEX.bootCI))
}

###############################################################################
###                                                                         ###
### An example illustrating the implementation for a dataset of the methods ###
...
## Details: 
  ...
  # In the dataset dat.end the event times are imputed at the end of the  
  # interval between the 2nd last and last physician visits. However, your data  
  # could have instead event times imputed at the middle of these intervals.   
  # In this case, the step below of data transformation to impute event times   
  # at the interval middle with the function imputeEventMid can be omitted.
...
# The default value for argument maxTimeCor is used by leaving it to NULL
plot.diag(data.middle = dat.mid, SIMEX_steps1_2.out = SIMEX12.out, 
          maxTimeCor = NULL)

### STEP b): If no major departures from linearity - for some interval between 
  #  0 and a point beyond the vertical black dotted line - is observed on the
  #  graph, choose the linear function as the default option. 
  #  => The linear function is selected for this example.
...
### STEP c): If the linear function is rejected in step b), select the 
  ...
  #  => Step c) is omitted for this example because the linear function was
  #     selected in step b).

### STEP d): Identify an approximate upper bound p'·delta.t on x axis, beyond the  
  #  black vertical dotted line, over which the function chosen in step b) or c)   
  #  mimics the corresponding correlations.

  #  => For this example, we selected the upper bound (p'·delta.t) = 14.

  # Add a dashed bold blue vertical line at the selected upper bound (p'·delta.t) = 14
  lines(c(14, 14), c(0,1), lty=2, lwd=2, col=4)

  # Identify the number k of SIMEX iteration corresponding approximately to the
  # upper bound (p'·delta.t) selected, i.e. select the iteration k for which 
  # the mean difference between the modified times of two last visits for cases 
  # corresponds approximately to 4 times this threshold (p'·delta.t) selected,  
  # i.e. approximately to 14*4 = 56 
